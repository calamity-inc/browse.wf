...
ExportMissionTypes_promise.then(res => { window.ExportMissionTypes = res; });
ExportFactions_promise.then(res => { window.ExportFactions = res; });
function formatExpiry(expiry) {
    expiry -= expiry % 1000;
    expiry += 1000; // normalise the ms so everything ticks at the same time
    const time = Date.now();
...
function updateWeeklyLocalised() {
    ...
    function createArchimedeaTooltip(keyPrefix, rawValue, descTransform) {
        ...
        if (text && desc) {
            ...
            addTooltip(abbr, finalDesc);
            return abbr;
        }
        else if (text) {
            // Have text but no description - use text without tooltip
            if (!desc) {
            ...
            // No localization available - use raw value
            console.warn("Missing osdict key:", key);
            return document.createTextNode(rawValue);
        }
    }
    // Transform function for frame variable descriptions
    function transformFrameVariable(desc, rawValue) {
    ...
    }
    {
        ...
        for (const mission of window.weekly.labConquestMissions) {
            ...
            }
            {
                ...
                td.appendChild(createArchimedeaTooltip("/Lotus/Language/Conquest/MissionVariant_LabConquest_", mission.variant));
                tr.appendChild(td);
            }
            for (let i = 0; i != 2; ++i) {
                const td = document.createElement("td");
    ...
    }
    {
        ...
        for (const mission of window.weekly.hexConquestMissions) {
            ...
            {
                const td = document.createElement("td");
                td.appendChild(createArchimedeaTooltip("/Lotus/Language/Conquest/MissionVariant_HexConquest_", mission.variant));
                tr.appendChild(td);
            }
            for (let i = 0; i != 2; ++i) {
                const td = document.createElement("td");
...
function updateWeekly() {
    ...
    Promise.all([dicts_promise, ExportMissionTypes_promise]).then(() => {
        ...
        const transformMissions = (conquest, conquestType) => {
            const missions = [];
